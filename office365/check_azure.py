...
'''
21.11.2018 modified by Roland Mueller for Azure check_azure
eg. /usr/local/bin/check_azure.py -H https://azure.microsoft.com/status/feed/ -T 24
...
def main(argv=None):
 ...
 Copyright 2011 John Wesorick (http://john.wesorick.com)"""
    ...
    feeddate = last_entry['published_parsed']
    title = last_entry['title']
    description = html2text(last_entry['description'])
    ...
    if (args.verbosity == '0'):
        ...
        output = 'Posted %s hrs ago - %s %s' % (hourssinceposted, title, link)
    elif (args.verbosity == '2'):
        output = 'Posted %s hrs ago - %s %s \n %s' % (hourssinceposted, title, link, description)
